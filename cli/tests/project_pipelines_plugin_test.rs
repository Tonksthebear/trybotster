...
fn catalog_plugin_project_pipelines_entity_contract_covers_registered_types_and_screen_bindings() {
    ...
    let result: String = lua
        .load(format!(
            ...
            local function screen_contract(screen_name)
              local sections = assert(contract.screens[screen_name], "missing screen contract: " .. screen_name)
              local source_counts = {{}}
              local sources = {{}}
              local fields = {{}}
              ...
              return source_counts, sources, fields, where_fields_by_source
            end

            local function assert_screen_bindings(screen_name, path)
              local body = read(path)
              local contract_source_counts, contract_sources, contract_fields, contract_where_fields_by_source =
                screen_contract(screen_name)
              ...
              for line in body:gmatch("[^\n]+") do
                ...
                if source then
                  ...
                  assert(contract_sources[source],
              ...
              for field in body:gmatch('ui%.bind%("%@/([%w_]+)"%)') do
                assert(contract_fields[field], screen_name .. " bound field missing from contract: " .. field)
              end
              for field in body:gmatch('ui%.bind%([^%)]*"%/([%w_]+)"') do
                assert(contract_fields[field], screen_name .. " bound field path missing from contract: " .. field)
              end
            end

            assert_screen_bindings("home", "{home_path}")
            assert_screen_bindings("pipelines", "{pipelines_path}")
            assert_screen_bindings("project", "{project_path}")
            assert_screen_bindings("ticket", "{ticket_path}")
