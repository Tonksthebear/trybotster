...
use botster_hub::relay::{SignalProtocolManager, PreKeyBundleData, SIGNAL_PROTOCOL_VERSION};
use std::env;
...
async fn test_new_manager_generates_prekey_bundle() {
    ...
    let manager = SignalProtocolManager::new(&hub_id).await
        .expect("Failed to create manager");

    let bundle = manager.build_prekey_bundle_data(1).await
...
async fn test_manager_persistence_same_identity() {
    ...
    let loaded_identity = {
        let manager = SignalProtocolManager::load_or_create(&hub_id).await
...
async fn test_prekey_consumption_finds_next() {
    ...
    let bundle1 = manager.build_prekey_bundle_data(1).await
    ...
    let bundle2 = manager.build_prekey_bundle_data(1).await
    ...
    let bundle3 = manager.build_prekey_bundle_data(999).await
...
async fn test_registration_id_stable() {
    ...
    assert_eq!(original_reg_id, loaded_reg_id,
...
async fn test_prekey_bundle_serialization() {
    ...
    // Deserialize back
    let parsed: PreKeyBundleData = serde_json::from_str(&json)
...
async fn test_different_hubs_different_identities() {
    ...
    let manager1 = SignalProtocolManager::new(&hub_id1).await
        .expect("Failed to create manager 1");
    let manager2 = SignalProtocolManager::new(&hub_id2).await
        .expect("Failed to create manager 2");

    let bundle1 = manager1.build_prekey_bundle_data(1).await
        .expect("Failed to build bundle 1");
    let bundle2 = manager2.build_prekey_bundle_data(1).await
        .expect("Failed to build bundle 2");

    assert_ne!(bundle1.identity_key, bundle2.identity_key,
        "Different hubs should have different identity keys");
    assert_ne!(bundle1.registration_id, bundle2.registration_id,
...
async fn test_signed_prekey_has_signature() {
    ...
    let sig_bytes = STANDARD.decode(&bundle.signed_prekey_signature)
...
async fn test_kyber_prekey_present() {
    ...
    use base64::{engine::general_purpose::STANDARD, Engine};
    let kyber_bytes = STANDARD.decode(&bundle.kyber_prekey)
        .expect("Kyber key should be valid base64");

    // libsignal serializes with a type prefix byte
    assert!(kyber_bytes.len() >= 1568, "Kyber1024 public key should be at least 1568 bytes, got {}", kyber_bytes.len());
}
...
async fn test_concurrent_manager_access() {
    ...
    let bundle2 = {
        let loaded = SignalProtocolManager::load_or_create(&hub_id_clone).await
            .expect("Failed to load manager");
        loaded.build_prekey_bundle_data(1).await
            .expect("Failed to build bundle 2")
    };
