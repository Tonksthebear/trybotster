...
// Ensure BOTSTER_ENV is set before any test code runs
static INIT: Once = Once::new();

fn ensure_test_env() {
    INIT.call_once(|| {
        env::set_var("BOTSTER_ENV", "test");
    });
}
...
mod config_tests {
    ...
    fn has_token_returns_false_when_empty() {
        let (_temp_dir, _guard) = setup_test_env();
        // No token set via env vars

        let config = Config::load().unwrap();
        assert!(
            !config.has_token(),
            "has_token should return false when token is empty"
        );
    }
    ...
    fn has_token_returns_true_with_token() {
        ...
        assert!(
            config.has_token(),
            "has_token should return true when token is set"
        );
    }
    ...
    fn get_api_key_returns_empty_string_when_no_token() {
        ...
        assert_eq!(
            config.get_api_key(),
            "",
            "should return empty string when no token"
        );
    }
}
