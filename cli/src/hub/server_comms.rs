...
mod tests {
    ...
    fn test_backpressure_recovery_cooldown_dispatches_rpc_snapshot_to_peer_sender() {
        ...
        let command = hub.tokio_runtime.block_on(async {
            tokio::time::timeout(Duration::from_secs(2), peer_rx.recv())
                .await
                .expect("timed out waiting for recovery snapshot")
                .expect("recovery sender closed")
        });
        match command {
            crate::worker::webrtc::WebRtcAdapterCommand::Pty {
    ...
    #[test]
    fn test_backpressure_recovery_missing_session_does_not_dispatch_or_count() {
        let (mut hub, _request_tx, _output_rx) = e2e_hub();
        ...
        assert!(!metrics
            .counters
            .contains_key("snapshot.backpressure_recovery.failed"));
    }

    #[test]
    fn test_backpressure_recovery_empty_rpc_snapshot_counts_empty_without_dispatch() {
        // A spawn_blocking JoinError would require aborting the runtime that
        // owns this test. The deterministic failure coverage is the reachable
        // empty snapshot plus full/closed peer-send branches above.
        let (mut hub, _request_tx, _output_rx) = e2e_hub();
        let session_uuid = "sess-recovery-empty";
        hub.handle_cache
            .add_session(test_session_backed_handle(session_uuid, 24, 80));
        let browser_identity = "browser-recovery-empty";
        let mut peer_rx = hub
            .webrtc
            .install_test_recovery_sender(browser_identity, &hub.tokio_runtime);
        let key = format!("{browser_identity}:{session_uuid}");
        hub.webrtc.record_backpressure_recovery(
            ...
            crate::worker::webrtc::BackpressureRecoveryEntry {
                browser_identity: browser_identity.to_string(),
                session_uuid: session_uuid.to_string(),
                subscription_id: "sub-recovery-empty".to_string(),
                last_drop: Instant::now() - crate::worker::webrtc::BACKPRESSURE_SNAPSHOT_COOLDOWN,
            },
        );
        ...
        hub.tokio_runtime.block_on(async {
            ...
            loop {
                ...
                if metrics
                    .counters
                    .get("snapshot.backpressure_recovery.empty")
                    .copied()
                    == Some(1)
                {
                    break;
                ...
                assert!(
                    ...
                    "timed out waiting for empty recovery counter"
                );
        ...
        assert!(peer_rx.try_recv().is_err());
        let metrics = hub.hub_event_metrics.snapshot();
        assert_eq!(metrics.counters["snapshot.backpressure_recovery.empty"], 1);
        assert!(!metrics
            .counters
            .contains_key("snapshot.backpressure_recovery.sent"));
    }
