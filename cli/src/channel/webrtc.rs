...
impl WebRtcChannel {
    ...
    /// Send signaling data to Rails (offer, answer, or ICE candidate).
    async fn send_signal(
        ...
        let url = format!("{}/hubs/{}/webrtc_signals", self.server_url, hub_id);
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| ChannelError::ConnectionFailed(format!("Failed to send signal: {e}")))?;
    ...
    /// Poll for pending signals from Rails.
    async fn poll_signals(&self, hub_id: &str, browser_identity: &str) -> Result<Vec<serde_json::Value>, ChannelError> {
        let url = format!(
            "{}/hubs/{}/webrtc_signals?browser_identity={}",
            self.server_url, hub_id, browser_identity
        );
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| ChannelError::ConnectionFailed(format!("Failed to poll signals: {e}")))?;
        ...
        let resp: SignalResponse = response
            .json()
            .await
            .map_err(|e| ChannelError::ConnectionFailed(format!("Failed to parse signals: {e}")))?;
    ...
    fn setup_data_channel_handlers(
        ...
        dc.on_message(Box::new(move |msg: DataChannelMessage| {
            ...
            let browser_identity = browser_identity.clone();
            ...
            Box::pin(async move {
                ...
                let decrypted = if let Some(ref cs) = crypto_service {
                    ...
                    match serde_json::from_slice::<SignalEnvelope>(&data) {
                        Ok(envelope) => {
                            match cs.decrypt(&envelope).await {
                ...
                let decompressed = if config_guard.as_ref().is_some_and(|c| c.compression_threshold.is_some()) {
