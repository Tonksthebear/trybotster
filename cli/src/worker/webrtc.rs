...
impl WebRtcTransportRunner {
    ...
    pub(crate) fn handle_plaintext_payload(&self, payload: &[u8]) -> WebRtcIngressOutcome {
        ...
        let client_message = self.ingress_to_client(TransportIngress::Json(msg));
        let msg = match client_message {
            ClientWorkerMessage::ControlFrame(ClientControlFrame::Json(value)) => value,
        ...
        match msg.get("type").and_then(|t| t.as_str()) {
            Some("dc_ping") => {
                ...
                WebRtcIngressOutcome::PongQueued
            }
            Some("dc_pong") => WebRtcIngressOutcome::PongObserved,
