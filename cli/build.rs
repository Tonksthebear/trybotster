...
// Prefer .expect over .unwrap so clippy::unwrap_used stays green
// without a file-wide allow (punch list M2).
...
fn main() {
    let out_dir = env::var("OUT_DIR").expect("build.rs fail-fast");
...
fn generate_embedded_lua(dest_path: &Path) {
    ...
    let mut output = File::create(dest_path).expect("build.rs fail-fast");

    writeln!(output, "// Auto-generated by build.rs - do not edit").expect("build.rs fail-fast");
    writeln!(output, "// Embeds all Lua files from cli/lua/").expect("build.rs fail-fast");
    writeln!(output).expect("build.rs fail-fast");
    ...
    .expect("build.rs fail-fast");
    writeln!(output, "///").expect("build.rs fail-fast");
    writeln!(output, "/// # Arguments").expect("build.rs fail-fast");
    writeln!(output, "///").expect("build.rs fail-fast");
    writeln!(
        output,
    ...
    .expect("build.rs fail-fast");
    writeln!(output, "///").expect("build.rs fail-fast");
    writeln!(output, "/// # Returns").expect("build.rs fail-fast");
    writeln!(output, "///").expect("build.rs fail-fast");
    writeln!(output, "/// The file content if found, None otherwise.").expect("build.rs fail-fast");
    writeln!(output, "#[allow(dead_code)]").expect("build.rs fail-fast");
    writeln!(
        output,
        "pub fn get_embedded_lua(path: &str) -> Option<&'static str> {{"
    )
    .expect("build.rs fail-fast");
    writeln!(output, "    EMBEDDED_LUA_FILES").expect("build.rs fail-fast");
    writeln!(output, "        .iter()").expect("build.rs fail-fast");
    writeln!(output, "        .find(|(p, _)| *p == path)").expect("build.rs fail-fast");
    writeln!(output, "        .map(|(_, content)| *content)").expect("build.rs fail-fast");
    writeln!(output, "}}").expect("build.rs fail-fast");
...
fn generate_stub_lua(dest_path: &Path) {
    ...
    .expect("build.rs fail-fast");
    writeln!(output, "    None").expect("build.rs fail-fast");
    writeln!(output, "}}").expect("build.rs fail-fast");
...
fn compile_terminfo(out_dir: &str) {
    ...
    writeln!(out, "// Auto-generated by build.rs — do not edit").expect("build.rs fail-fast");
    writeln!(
        out,
        "/// Pre-compiled terminfo entries: (relative_path, compiled_bytes)."
    )
    .expect("build.rs fail-fast");
    writeln!(out, "pub static TERMINFO_FILES: &[(&str, &[u8])] = &[").expect("build.rs fail-fast");
    for (rel_path, abs_path) in &compiled {
        writeln!(out, "    (\"{rel_path}\", include_bytes!(\"{abs_path}\")),",).expect("build.rs fail-fast");
    }
    writeln!(out, "];").expect("build.rs fail-fast");
}
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            let rel = path
                ...
                .to_string_lossy()
                .to_string();
            let abs = path.canonicalize().expect("build.rs fail-fast").to_string_lossy().to_string();
            files.push((rel, abs));
        }
    }
}
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                ...
                let abs_path = path.canonicalize().expect("build.rs fail-fast").to_string_lossy().to_string();
