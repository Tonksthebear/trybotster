...
module Api
  ...
  class DevicesController < ApplicationController
    ...
    #
    # Params:
    # - device_type: "cli" or "browser" (required)
    # - name: human-readable device name (required)
    # - fingerprint: device fingerprint for identification (required for CLI)
    # - public_key: optional - only sent if server_assisted_pairing is enabled
    # - server_assisted_pairing: boolean flag indicating mode
    def create
      ...
      device.assign_attributes(
        ...
        public_key: public_key, # May be nil in secure mode
    ...
    # Authenticate via API key (CLI) or session (browser)
    ...
    # Return the authenticated user (from API key or session)
