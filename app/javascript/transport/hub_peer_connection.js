...
class HubPeerConnection {
  ...
  async #handleIceCandidate(hubId, candidateData) {
    const conn = this.#connections.get(hubId)
    if (!conn) return // No signaling connection for this hub

    const candidate = new RTCIceCandidate(candidateData)

    // Remote ICE can arrive before we've created/attached the RTCPeerConnection
    // (offer in progress) or before setRemoteDescription(answer). Buffer instead
    // of dropping to avoid "unknown browser"/slow-connect churn.
  ...
  #routeControlMessage(hubId, msg) {
    ...
    } else if (msg.type === "health" || msg.type === "dc_ping" || msg.type === "dc_pong") {
      // Connection-level control messages without subscriptionId.
      // Fan out to all local subscriptions for this hub so each typed
      // Connection instance can process health + heartbeat frames.
      const conn = this.#connections.get(hubId)
      if (conn) {
