...
- Reference other fixtures by name: `user: primary_user`, `device: cli_device`
...
4. Reference by name in tests: `users(:primary_user)`, `hubs(:active_hub)`
...
Key methods:
- `auth_headers_for(:primary_user)` — creates a device + token dynamically, returns `Authorization: Bearer ...` headers
...
class ThingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "lists things for authenticated user" do
    get things_url, headers: auth_headers_for(:primary_user)
    assert_response :ok
